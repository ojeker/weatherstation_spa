...
|------|------|-------------|
| 1.1 | **SCAFFOLD** | Set up Vue 3 project with Vite, TypeScript, Vitest, Cypress, and folder structure (`src/domain`, `src/application`, `src/infrastructure`, `src/ui`) |
...
| 6.3 | **UI-CURRENT-CARD** | Build `CurrentReadingCard` component (temperature, sunshine, precipitation, timestamp) |
| 6.4 | **UI-HOURLY-CHARTS** | Build hourly charts (temperature, sunshine, precipitation) using a chart library |
| 6.5 | **UI-LOADING-STATE** | Implement loading spinner/skeleton |
...
| 6.9 | **UI-RESPONSIVE** | Add responsive styles for mobile/tablet/desktop |

## Phase 7: Component Tests

| Step | Name | Description |
|------|------|-------------|
| 7.1 | **TEST-CURRENT-CARD** | Component tests for CurrentReadingCard (formatting, null values) |
...
```
SCAFFOLD → DOMAIN-* → INFRA-* → APP-* → UI-*
                ↓           ↓         ↓
           TEST-FIXTURES → TEST-* → E2E-*
```
